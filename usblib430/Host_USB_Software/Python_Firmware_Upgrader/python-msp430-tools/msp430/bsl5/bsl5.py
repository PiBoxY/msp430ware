...
class BSL5(object):
    ...
    def reset(self):
        ...
        """
        # try a write PMMCTL0 register
        try:
            #Initiate a software BOR reset by writing 0xA506 to PMMCTL0 register.
            self.BSL_RX_DATA_BLOCK_FAST(0x0120, "\xA5\x06")
            #The following is a dummy command to initiate buffer flush.  RAM_BSL files greater than .06 requires the host
            #to send a read command, a CRC, a write command or a reset command to a new location in order to trigger a buffer flush.  This
            #is because BSL buffers all data until a 128 byte block is received and then writes the data to device.  If an 
            #image did not end on a block boundary the data is not transferred to the device.

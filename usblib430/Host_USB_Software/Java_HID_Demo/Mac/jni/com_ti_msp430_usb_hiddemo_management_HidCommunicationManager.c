...
int getInterfaceFromPath(struct hid_device_info *devs) {
    
    const char* path = devs->path;
    const char* subStr = "HID Interface@";
...
(JNIEnv * env, jobject obj, jint vid, jint pid) {
    ...
    while (cur_dev) {
        ...
        if(cur_dev->vendor_id == vid && cur_dev->product_id == pid) {
            ...
            buf[count] = getInterfaceFromPath(cur_dev);
...
(JNIEnv * env, jobject obj, jint vid, jint pid, jstring javaserial, jint device) {
    ...
    devs = hid_enumerate(0x0, 0x0);
    cur_dev = devs;
    const char* serial = (*env)->GetStringUTFChars(env,javaserial,NULL);

    //printf("connectDeviceNative\n");
    //printf("Want vid %X, pid %X serial %s device %d\n", vid, pid, serial, device);
    ...
    while (cur_dev) {
        ...
        if(cur_dev->vendor_id == vid && cur_dev->product_id == pid) {
            ...
            int cur_interface = getInterfaceFromPath(cur_dev);
            if(device >= 0 && cur_interface != device) {
                cur_dev = cur_dev->next;
                continue;
            }
...
(JNIEnv * env, jobject obj, jlong longhandle) {
...
(JNIEnv * env, jobject obj, jlong longhandle, jbyteArray data, jint size) {

...
(JNIEnv * env, jobject obj, jint vid, jint pid, jstring javaserial, jint device) {
    ...
    while (cur_dev) {
        ...
        //const char* debugSerial=(*env)->GetStringUTFChars(env,javaserial,0);
        //printf("    Checking vid %X, pid %X serial %s device %d\n", cur_dev->vendor_id, 
        //        cur_dev->product_id, debugSerial, cur_dev-> interface_number); 

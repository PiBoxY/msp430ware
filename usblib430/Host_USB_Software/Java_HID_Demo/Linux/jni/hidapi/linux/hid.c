...
static int openDevHandle = 0;
static hid_device* devHandles[10];
...
hid_device * HID_API_EXPORT hid_open_path(const char *path)
{
	...
	if (dev->device_handle > 0) {
        ...
        // Instead of returning the handle we return the index into the devHandles array.
        // This is because JAVA only wants to store the handles as 4 byte int's instead
        // of 8 byte pointers.
		//return dev;
        return (hid_device*)openDevHandle;
	}
	else {
		// Unable to open any devices.
		printf("Unable to open any devices.\n");
		free(dev);
        openDevHandle--;
		return NULL;
	}
...
int HID_API_EXPORT hid_write(hid_device *dev, const unsigned char *data, size_t length)
{
	...
	if (bytes_written < 0) {
        switch (errno) {
             case EAGAIN: printf("errno is EAGAIN\n"); break;
             case EBADF: printf("errno is EBADF\n"); break;
             case EFBIG: printf("errno is EFBIG\n"); break;
             case EINTR: printf("errno is EINTR\n"); break;
             case EIO: printf("errno is EIO\n"); break;
             case ENOSPC: printf("errno is ENOSPC\n"); break;
             case EINVAL: printf("errno is EINVAL\n"); break;
             case ETIMEDOUT: printf("errno is ETIMEDOUT\n"); break;
             default: printf("errno is UNKNOWN: %d\n", errno);
        }
    }

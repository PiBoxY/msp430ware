 ...
 * /                   Added int32_t file name support.
...
static
uint16_t inc_lock (                                                     /* Increment file open counter and returns its index (0:int
                                                                     *error) */
    ...
    )
{
    ...
    for (;;){
        ...
        if (di >= _MAX_LFN){                                                /* Reject too int32_t name */
            return (FR_INVALID_NAME);
        }
#if !_LFN_UNICODE
        w &= 0xFF;
    ...
    )
{
    ...
    for (cc = res = 0; cc != EOF; res += cc){
        ...
        if (c == 'l' || c == 'L'){                                  /* Prefix: Size is int32_t int */
            f |= 2; c = *str++;
        }
        if (!c){
            break;
        ...
        val =
            (f & 2) ? va_arg(arp,
                int32_t) : ((d == 'D') ? (int32_t)va_arg(arp, int16_t) : va_arg(arp,
                             uint16_t));

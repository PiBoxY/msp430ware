...
/* Misc definitions */
#define LD_CLUST(dir)   (((uint32_t)LD_WORD(dir + \
                              DIR_FstClusHI) << \
                          16) | LD_WORD(dir + DIR_FstClusLO))
#define ST_CLUST(dir,cl) {ST_WORD(dir + DIR_FstClusLO, cl); ST_WORD( \
                              dir + DIR_FstClusHI, \
                              (uint32_t)cl >> 16);}
...
#define IsDBCS1(c)  (((uint8_t)(c) >= _DF1S && \
                      (uint8_t)(c) <= _DF1E) || \
                     ((uint8_t)(c) >= _DF2S && (uint8_t)(c) <= _DF2E))
#else                       /* One 1st byte area */
#define IsDBCS1(c)  ((uint8_t)(c) >= _DF1S && (uint8_t)(c) <= _DF1E)
#endif

#ifdef _DS3S                /* Three 2nd byte areas */
#define IsDBCS2(c)  (((uint8_t)(c) >= _DS1S && \
                      (uint8_t)(c) <= _DS1E) || \
                     ((uint8_t)(c) >= _DS2S && \
                      (uint8_t)(c) <= _DS2E) || \
                     ((uint8_t)(c) >= _DS3S && (uint8_t)(c) <= _DS3E))
#else                       /* Two 2nd byte areas */
#define IsDBCS2(c)  (((uint8_t)(c) >= _DS1S && \
                      (uint8_t)(c) <= _DS1E) || \
                     ((uint8_t)(c) >= _DS2S && (uint8_t)(c) <= _DS2E))
#endif
...
static
int16_t enq_lock (                                                      /* Check if an entry is available for a new file */
    FATFS* fs                                                       /* File system object */
    )
{
    uint16_t i;
...
static
uint16_t inc_lock (                                                     /* Increment file open counter and returns its index (0:int16_t
                                                                     *error) */
    ...
    )
{
    ...
    if (i == _FS_SHARE){                                            /* Not opened. Register it as new. */
        for (i = 0; i < _FS_SHARE && Files[i].fs; i++){}
        if (i == _FS_SHARE){
            return ( 0) ;                                           /* No space to register (int16_t err) */
        }
        Files[i].fs = dj->fs;
        Files[i].clu = dj->sclust;
        Files[i].idx = dj->index;
    ...
    if (acc && Files[i].ctr){
        return ( 0) ;                                               /* Access violation (int16_t err) */
    }
...
#if !_FS_READONLY
static
uint32_t create_chain (                                                    /* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk
                                                                         *error, >=2:New cluster# */
    FATFS *fs,                                                          /* File system object */
    ...
    )
{
    ...
    if (clst == 0){                                                     /* Static table (root-dir in FAT12/16) */
        ...
        while (idx >= ic){                                              /* Follow cluster chain */
            ...
            if (clst < 2 || clst >= dj->fs->n_fatent){                  /* Reached to end of table or int16_t error */
                return (FR_INT_ERR);
            }
            idx -= ic;
        }
...
static
FRESULT dir_next (                                                      /* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:EOT
...
static
FRESULT dir_read (
    DIR *dj                                                                     /* Pointer to the directory object that pointing the
                                                                                 *entry to be read */
    )
{
    FRESULT res;
    ...
    while (dj->sect){
        ...
        a = dir[DIR_Attr] & AM_MASK;
        if (c == 0xE5 ||
            (!_FS_RPATH && c == '.') || ((a & AM_VOL) && a != AM_LFN)){         /* An entry without valid data */
            ord = 0xFF;
        } else {
            if (a == AM_LFN){                                                   /* An LFN entry is found */
                if (c & 0x40){                                                  /* Is it start of LFN sequence? */
...
#if !_FS_READONLY
static
FRESULT dir_register (                                                      /* FR_OK:Successful, FR_DENIED:No free entry or too many
                                                                             *SFN collision, FR_DISK_ERR:Disk error */
    ...
    )
{
    ...
    if (sn[NS] & NS_LOSS){                                                  /* When LFN is out of 8.3 format, generate a numbered
                                                                             *name */
    ...
    if (sn[NS] & NS_LFN){                                                   /* When LFN is to be created, reserve an SFN + LFN
                                                                             *entries. */
        for (ne = 0; lfn[ne]; ne++){}
        ne = (ne + 25) / 13;
    ...
    )
{
    ...
    *path = &p[si];                                                         /* Return pointer to the next segment */
    ...
    if (dj->fn[0] == 0xE5){
        dj->fn[0] = 0x05;                                                   /* If the first char collides with deleted mark, replace
                                                                             *it with 0x05 */
    }
    if (ni == 8){
        b <<= 2;
    ...
    if (!(cf & NS_LFN)){                                                    /* When LFN is in 8.3 format without extended char, NT
                                                                             *flags are created */
        if ((b & 0x03) == 0x01){
    ...
    *path = &p[si];                                                         /* Return pointer to the next segment */
    ...
    if (sfn[0] == 0xE5){
        sfn[0] = 0x05;                                                      /* When first char collides with 0xE5, replace it with
                                                                             *0x05 */
    }
    if (ni == 8){
        b <<= 2;
...
static
uint8_t check_fs (                                                         /* 0:The FAT BR, 1:Valid BR but not an FAT, 2:Not a BR,
                                                                         *3:Disk error */
    FATFS *fs,                                                          /* File system object */
    ...
    )
{
    ...
    if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55){              /* Check record signature (always placed at offset 510 even if the
                                                             *sector size is >512) */
        return (2);
    }
    ...
    )
{
    ...
    *rfs = fs = FatFs[vol];                                             /* Return pointer to the corresponding file system object */
    ...
    if (fs->fs_type){                                                   /* If the logical drive has been mounted */
        ...
        if (!(stat & STA_NOINIT)){                                      /* and the physical drive is kept initialized (has not been
                                                                         *changed), */
#if !_FS_READONLY
    ...
    if (LD_WORD(fs->win + BPB_BytsPerSec) != SS(fs)){                   /* (BPB_BytsPerSec must be equal to the physical sector
                                                                         *size) */
...
FRESULT f_mount (
    ...
    FATFS *fs                                                               /* Pointer to new file system object (NULL for
                                                                             *unmount)*/
    )
{
    FATFS *rfs;
        ...
        if ((fp->fptr % SS(fp->fs)) == 0){                                  /* On the sector boundary? */
            ...
            if (cc){                                                        /* Read maximum contiguous sectors directly */
                ...
                if (fp->fs->wflag && fp->fs->winsect - sect < cc){
                    mem_cpy(rbuff + ((fp->fs->winsect - sect) * SS(
                                         fp->fs)), fp->fs->win, SS(fp->fs));
                }
#else
                if ((fp->flag & FA__DIRTY) && fp->dsect - sect < cc){
                    mem_cpy(rbuff + ((fp->dsect - sect) * SS(
                                         fp->fs)), fp->buf, SS(fp->fs));
                }
#endif
#endif
...
#if _FS_TINY
            if (fp->fs->winsect == fp->dsect && move_window(fp->fs, 0)){ /* Write back data buffer prior to following direct
                                                                          *transfer */
                ABORT(fp->fs, FR_DISK_ERR);
            }
#else
            if (fp->flag & FA__DIRTY){                                      /* Write back data buffer prior to following direct
                                                                             *transfer */
                if (disk_write(fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK){
                    ABORT(fp->fs, FR_DISK_ERR);
                }
...
#if _FS_TINY
                if (fp->fs->winsect - sect < cc){                           /* Refill sector cache if it gets dirty by the direct
                                                                             *write */
                    mem_cpy(fp->fs->win, wbuff +
                        ((fp->fs->winsect - sect) * SS(fp->fs)), SS(fp->fs));
                    fp->fs->wflag = 0;
                }
#else
                if (fp->dsect - sect < cc){                                 /* Refill sector cache if it gets dirty by the direct
                                                                             *write */
                    mem_cpy(fp->buf, wbuff + ((fp->dsect - sect) * SS(
                                                  fp->fs)), SS(fp->fs));
                    fp->flag &= ~FA__DIRTY;
                }
#endif
    ...
    )
{
    ...
    if (fp->cltbl){                 /* Fast seek */
        uint32_t cl, pcl, ncl, tcl, dsc, tlen, *tbl = fp->cltbl;
        uint8_t csc;
            ...
            if (ofs){
                ...
                if (fp->fptr % SS(fp->fs) && dsc != fp->dsect){
                    ...
                    if (fp->flag & FA__DIRTY){  /* Flush dirty buffer if needed */
                        if (disk_write(fp->fs->drv, fp->buf, fp->dsect,
                                1) != RES_OK){
                            ABORT(fp->fs, FR_DISK_ERR);
                        }
                        fp->flag &= ~FA__DIRTY;
        ...
        if (res == FR_OK){                                                          /* Old object is found */
            if (!djo.dir){                                                          /* Is root dir? */
                ...
                if (res == FR_NO_FILE){                                             /*
                                                                                     ...
                                                                                     * Start critical section that any interruption
                                                                                     *or error can cause cross-link
                                                                                     */
...
FRESULT f_forward (
    ...
    uint16_t btr,                                                           /* Number of bytes to forward */
    ...
    )
{
    ...
    uint16_t i;
    uint32_t b_vol, b_fat, b_dir, b_data;                                  /* Offset (LBA) */
    ...
    for (i = 0; i < N_FATS; i++){
        ...
        for (n = 1; n < n_fat; n++){                                /* This loop may take a time on FAT32 volume due to many single
                                                                     *sector write */
            if (disk_write(drv, tbl, wsect++, 1) != RES_OK){
                return (FR_DISK_ERR);
            }
    ...
    return ((disk_ioctl(drv, CTRL_SYNC,
    ...
    )
{
    ...
    for (cc = res = 0; cc != EOF; res += cc){
        ...
        switch (d){                                                 /* Type is... */
            ...
            case 'C':                                               /* Character */
                cc = f_putc((TCHAR)va_arg(arp, int16_t), fil); continue;
            case 'B':                                               /* Binary */
                r = 2; break;
            case 'O':                                               /* Octal */
                r = 8; break;
        ...
        val =
            (f & 2) ? va_arg(arp,
                long) : ((d == 'D') ? (long)va_arg(arp, int16_t) : va_arg(arp,
                             uint16_t));

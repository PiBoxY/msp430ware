...
__no_init char RAM_TX_Buf[MAX_BUFFER_SIZE];
__no_init char RAM_RX_Buf[MAX_BUFFER_SIZE];

#define USCIB_I2C_VERSION 0x01
const unsigned char BSL430_PI_Version @ "BSL430_VERSION_PI" =
    (BSL430_eUSCIB_I2C_PI + USCIB_I2C_VERSION);
...
void PI_init()
{
    ...
    */
    ...
    CSCTL0_H = 0xFF; 
#endif
    PORT_SEL1 |= SDA + SCL;                                   // Assign I2C pins to USCI_B1
    PORT_SEL0 &= ~(SDA + SCL);                                   // Assign I2C pins to USCI_B1
    
    UCZNCTL1 |= UCSWRST;                                     // Enable SW reset
    UCZNCTL0 = UCMODE_3 + UCSYNC;                            // I2C Slave, synchronous mode
    UCZNI2COA = SLAVE_ADDRESS+UCOAEN;                               // Own Address is 048h
    UCZNCTL1 &= ~UCSWRST;                                    // Clear SW reset, resume operation

    UCZNIE |= UCTXIE0 + UCRXIE0 + UCSTPIE + UCSTTIE;

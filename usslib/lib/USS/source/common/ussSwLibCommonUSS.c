...
USS_message_code commonPrepareForUSSPowerUp
	(USS_SW_Library_configuration *config)
{
	// Check if USS is in ready state.
	// IF USS is not in READY STATE, then verify USS_OSC is stable
	if(false == (UUPSCTL & UPSTATE_3))
	{
		...
		USS_SW_Library_configuration *config)
{
	...
	if(((USSSWLIB_USS_interrupt_status & (0xFFFF- USS_SAPH_PING_DN_INTERRUPT)) == USS_SAPH_SEQ_ACQ_DN_INTERRUPT) ||

												...
												uint16_t lowPowerDelay)
{
	...
	{
	case USS_measurement_capture_sequence_ch0:
    ...
    #error "USS Lib Measurement not supported or defined for EVM configuration"
		...
		break;
	case USS_measurement_capture_sequence_ch1:
#if(USSSWLIB_ENABLE_EXTERNAL_CIRCUITRY_CONTROL == false)
		...
		USS_SW_Library_configuration *config)
{
	...
	if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
			code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,0);
		}
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
			code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,0);
		}
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch1_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch0_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch0_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch1_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch1_ch0_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch0_ch1_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch0_ch0_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch1_ch1_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,config->measurementConfig->restartLowPowerCaptureCount);
		if(code == USS_message_code_no_error)
		{
	...
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch0,0);
	}
	else if(config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1)
	{
		code = USS_commonTriggerSingleCapture(config,USS_measurement_capture_sequence_ch1,0);
	}
	else
...
USS_message_code USS_swapCaptureChannels(
        USS_SW_Library_configuration *config)
{
    // Unlock SAPH
    GENERIC_SAPH_KEY = KEY;
    ...
    {
    case USS_measurement_capture_sequence_ch0:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1;
        break;
    case USS_measurement_capture_sequence_ch0_ch0:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch1;
        break;
    case USS_measurement_capture_sequence_ch0_ch0_ch0:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch1_ch1;
        break;
    case USS_measurement_capture_sequence_ch0_ch0_ch0_ch0:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch1_ch1_ch1;
        break;
    case USS_measurement_capture_sequence_ch0_ch1:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch0;
        break;
    case USS_measurement_capture_sequence_ch0_ch1_ch0:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch0_ch1;
        break;
    case USS_measurement_capture_sequence_ch0_ch1_ch0_ch1:
        GENERIC_SAPH_ASCTL0 |= (ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch1_ch0_ch1_ch0;
        break;
    case USS_measurement_capture_sequence_ch1:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0;
        break;
    case USS_measurement_capture_sequence_ch1_ch1:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch0;
        break;
    case USS_measurement_capture_sequence_ch1_ch1_ch1:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch0_ch0;
        break;
    case USS_measurement_capture_sequence_ch1_ch1_ch1_ch1:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch0_ch0_ch0;
        break;
    case USS_measurement_capture_sequence_ch1_ch0:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch1;
        break;
    case USS_measurement_capture_sequence_ch1_ch0_ch1:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch1_ch0;
        break;
    case USS_measurement_capture_sequence_ch1_ch0_ch1_ch0:
        GENERIC_SAPH_ASCTL0 &= ~(ASQCHSEL_1);
        config->measurementConfig->sequenceSelection = USS_measurement_capture_sequence_ch0_ch1_ch0_ch1;
        break;
    default:
        break;
    }

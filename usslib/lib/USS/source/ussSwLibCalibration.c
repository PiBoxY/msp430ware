		...
		USS_calibration_hspll_results *testResults)
{
	...
	if(timeout > 0)
	{
	    // Configure timer to measure period between Event 1 at count = 2
	    // and Event #2 at count = 1002
                          ...
                          USS_dTof_absTof_offset_test_config *testConfig)
{
    ...
    if(((config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch0_ch1) ||
       (config->measurementConfig->sequenceSelection == USS_measurement_capture_sequence_ch1_ch0)) == false){
        return USS_message_code_Calibration_Offset_invalid_configuration;
    }

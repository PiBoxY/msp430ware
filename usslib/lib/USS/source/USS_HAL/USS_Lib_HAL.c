...
{
    // This interrupt is only used for TAxCCTL1.CCIFG, not any other TAxCCTLx
    // or TAIFG.
    // The interrupt vector is not checked to maintain same/similar number
    // of cycles between CCR0 and CCR1 events.
    USSSWLIB_USS_interrupt_status |= (USS_TIMER_EXPIRED_EVENT1_INTERRUPT);
    HARDWAREWREG16(USSSWLIB_TIMER_BASE_ADDRESS + OFS_TAxCCTL1) &= ~(CCIFG);

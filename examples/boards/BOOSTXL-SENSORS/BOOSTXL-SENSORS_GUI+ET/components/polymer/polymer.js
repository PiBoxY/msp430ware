...
(function (global) {
    ...
    function MemberExpression(object, property, accessor) {
        ...
        this.simplePath = !this.dynamicDeps &&
        ...
        if (useNative) {
            ...
            (function () {
                if (document.readyState === 'loading') {
                    ...
                    for (var i = 0, l = imports.length, imp;
                        (i < l) && (imp = imports[i]); i++) {
                        handleImport(imp);
                    }
                }
...
(function (global) {
    ...
    Path.prototype = createObject({
        ...
        setValueFrom: function (obj, value) {
            ...
            //<<<Boualem-----------------------------------------------------------       
            if (obj && obj.parentNode && obj.parentNode.nodeName == "DESIGN-HOST" && this[i] == "id") {
                console.log("id changing from: " + obj.id + " to: " + value);
                var event = new Event('elementIdChange');
                event.oldValue = obj.id;
...
(function (scope) {
    ...
    DOMTokenList.prototype.add = function () {
        for (var i = 0; i < arguments.length; i++) {
            //console.log("---DOMTokenList.prototype.add called");
            add.call(this, arguments[i]);
        }
    };
    DOMTokenList.prototype.remove = function () {
        for (var i = 0; i < arguments.length; i++) {
            //console.log("---DOMTokenList.prototype.remove called");
            remove.call(this, arguments[i]);
        }
    };
    DOMTokenList.prototype.toggle = function (name, bool) {
        //console.log("---DOMTokenList.prototype.toggle called");
    ...
    DOMTokenList.prototype.switch = function (oldName, newName) {
        //console.log("---DOMTokenList.prototype.switch called");
...
(function (scope) {
    ...
    var utils = {
        ...
        //<<<Boualem------------
        classAdd: function (anew, className) {
            if (anew) {
                anew.classList.add(className);
            }
        },
        classToggle: function (anew, className) {
            if (anew) {
                anew.classList.toggle(className);
            }
        },
        classClear: function (element2ClearList, className) {
            if (element2ClearList) {
                for (var i = 0; i < element2ClearList.length; i++) {
                    var element2Clear = element2ClearList[i];
                    if (element2Clear) {
                        if (element2Clear.classList && element2Clear.classList.remove) {
                            element2Clear.classList.remove(className);
                        }

                    } else {
                        console.log("element2Clear=" + element2Clear);
                    }
                }
            }
        },
...
(function (scope) {
    ...
    var styles = {
        ...
        installScopeStyle: function (style, name, scope) {
            var scope = scope || this.findStyleScope(),
                name = name || '';
            if (scope && !this.scopeHasNamedStyle(scope, this.localName + name)) {
                var cssText = '';
                if (style instanceof Array) {
                    for (var i = 0, l = style.length, s;
                        (i < l) && (s = style[i]); i++) {
...
(function (scope) {
    ...
    */
    var queue = {
        ...
        isEmpty: function () {
            for (var i = 0, l = elements.length, e;
                (i < l) &&
                (e = elements[i]); i++) {

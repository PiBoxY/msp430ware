...
(function() {
	...
	var AgentHostApp = function() {
		...
		var AGENT_VERSION = "2.4";
		
		var EXT_VERSION = "2.0";

		if( getBrowser() === TICloudAgent.BROWSER.SAFARI ){
			EXT_VERSION = "2.1";
		} else if( getBrowser() === TICloudAgent.BROWSER.CHROME ) {
			// Even though Bridge extension version is now moved to 2.2
			// The content script is still injecting 2.0. This is intentional because
			// We do not control the updates for chrome. Everyone will pick up 2.2 automatically.
			// This implies that we have to maintain backwards compatability for at least 1 version.
			EXT_VERSION = "2.0";
		}
		...
		var getSteps = function(errors, target) {
			...
			for (var i = 0; i < errors.length; i++) {
				...
				if (getBrowser() === TICloudAgent.BROWSER.IE && errors[i] instanceof TICloudAgent.MissingExtension) {
					...
					steps.push(createInstallAgentStep(target, errors[i]));

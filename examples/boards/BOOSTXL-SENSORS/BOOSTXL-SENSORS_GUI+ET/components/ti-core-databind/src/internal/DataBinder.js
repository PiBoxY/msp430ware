...
(function()
{
    ...
    {
        var oldValue = this.srcBinding.getValue();
        if (oldValue)
        {
            this.onDataReceived(oldValue);
        }
    ...
    StreamingListener.prototype.onDataReceived = function(newValue)
    {
        ...
        this.destBinding.onDataReceived(newValue);
    };

    StreamingListener.prototype.onValueChanged = function()
    {
    };
    /* bc: Added getStatus method so that Binder.prototype.enable does not throw an exception when calling
    ...
    {
        if (srcBinding.addStreamingListener && targetBinding.onDataReceived)
        {
    ...
    gc.databind.internal.DataBinder.bind =
        function(targetBinding, modelBinding, getter, setter)
        {

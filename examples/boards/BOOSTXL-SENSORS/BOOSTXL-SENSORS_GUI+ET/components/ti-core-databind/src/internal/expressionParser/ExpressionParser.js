...
gc.databind.internal.expressionParser.ExpressionParser.prototype.bindValue = function(uri)
{
	if (uri.length === 0)
	{
		throw "Empty Param";
	...
	if (pos > 0)
	{
		...
		var modelFactory = this.bindProvider.getModel(modelName);
	...
	var defaultModelFactory = this.bindProvider.getModel();
	if (defaultModelFactory !== null)
	{
		return defaultModelFactory.createNewBind(uri);
	}
	return null;
};

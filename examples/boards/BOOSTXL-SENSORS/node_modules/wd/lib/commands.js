...
/**
 * performTouch(element, touchGestures) -> cb(err, touchStateObjects)
 * performTouch(touchGestures) -> cb(err, touchStateObjects)
 *
...
commands.performTouch = function () {
  ...
  var cb = fargs.callback,
      element = fargs.all[0],
      touchAction = fargs.all[1];
  if(!touchAction) {
    touchAction = element;
    element = null;
  }
  try {
    var gestures = touchAction._jsonWireGestures(element);
    _this._jsonWireCall({
      method: 'POST',
...
/**
 * performTouchAction(element, touchGestures) -> cb(err, touchStateObjects)
 * performTouchAction(touchGestures) -> cb(err, touchStateObjects)
 *
 *
 * @jsonWire POST /session/:sessionId/touch/perform
 */
commands.performTouchAction = commands.performTouch;

/**
 * performMultiTouch(element, actions) -> cb(err, touchStateObjects)
 * performMultiTouch(actions) -> cb(err, touchStateObjects)
 *
 * @jsonWire POST /session/:sessionId/touch/multi/perform
 */
commands.performMultiTouch = function () {
  var _this = this;
  var fargs = utils.varargs(arguments);
  var cb = fargs.callback,
      element = fargs.all[0],
  ...
  try {
    var gestures = multiTouchAction._jsonWireGestures(element);
    var data = {
      actions: gestures
    };
...
/**
 * performMultiTouchAction(element, actions) -> cb(err, touchStateObjects)
 * performMultiTouchAction(actions) -> cb(err, touchStateObjects)
 *
 * @jsonWire POST /session/:sessionId/touch/multi/perform
 */
commands.performMultiTouchAction = commands.performMultiTouch;
...
/**
 * pushFileToDevice(pathOnDevice, base64Data, cb) -> cb(isInstalled, err)
 *
...
/**
 * pushFile(pathOnDevice, base64Data, cb) -> cb(isInstalled, err)
 *
 * @jsonWire POST /session/:sessionId/appium/device/push_file
 */
 ...
 *
 * @jsonWire POST /appium/element/:elementId?/value
 */

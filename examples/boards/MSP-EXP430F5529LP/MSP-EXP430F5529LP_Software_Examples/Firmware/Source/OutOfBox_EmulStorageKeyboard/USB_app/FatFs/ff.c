...
/* Misc definitions */
#define LD_CLUST(dir)   (((DWORD)LD_WORD(dir + \
                              DIR_FstClusHI) << \
...
static
UINT inc_lock (                                                     /* Increment file open counter and returns its index (0:int
                                                                     *error) */
    ...
    )
{
    ...
    for (cc = res = 0; cc != EOF; res += cc){
        ...
        val =
            (f & 2) ? va_arg(arp,
                long) : ((d == 'D') ? (long)va_arg(arp, int) : va_arg(arp,
                             unsigned int));

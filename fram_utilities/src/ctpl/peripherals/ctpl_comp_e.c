...
void ctpl_COMP_E_save(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    storage[4] = HWREG16(baseAddress + OFS_CECTL1);
    HWREG16(baseAddress + OFS_CECTL1) &= ~CEON;
    storage[3] = HWREG16(baseAddress + OFS_CEINT);
    storage[2] = HWREG16(baseAddress + OFS_CECTL3);
    storage[1] = HWREG16(baseAddress + OFS_CECTL2);
    storage[0] = HWREG16(baseAddress + OFS_CECTL0);
...
void ctpl_COMP_E_restore(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    HWREG16(baseAddress + OFS_CECTL0) = storage[0];
    HWREG16(baseAddress + OFS_CECTL2) = storage[1];
    HWREG16(baseAddress + OFS_CECTL3) = storage[2];
    HWREG16(baseAddress + OFS_CEINT)  = storage[3];
    HWREG16(baseAddress + OFS_CECTL1) = storage[4];

    return;
}

#endif //__MSP430_HAS_COMP_E__

...
void ctpl_DMAX_6_save(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    storage[38] = HWREG16(baseAddress + OFS_DMA5CTL);
    storage[37] = HWREG16(baseAddress + OFS_DMA5SZ);
    *(uint32_t *)&(storage[35]) = __data16_read_addr(baseAddress + OFS_DMA5DA);
    *(uint32_t *)&(storage[33]) = __data16_read_addr(baseAddress + OFS_DMA5SA);
    storage[32] = HWREG16(baseAddress + OFS_DMA4CTL);
    storage[31] = HWREG16(baseAddress + OFS_DMA4SZ);
    *(uint32_t *)&(storage[29]) = __data16_read_addr(baseAddress + OFS_DMA4DA);
    *(uint32_t *)&(storage[27]) = __data16_read_addr(baseAddress + OFS_DMA4SA);
    storage[26] = HWREG16(baseAddress + OFS_DMA3CTL);
    storage[25] = HWREG16(baseAddress + OFS_DMA3SZ);
    *(uint32_t *)&(storage[23]) = __data16_read_addr(baseAddress + OFS_DMA3DA);
    *(uint32_t *)&(storage[21]) = __data16_read_addr(baseAddress + OFS_DMA3SA);
...
void ctpl_DMAX_6_restore(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    __data16_write_addr((baseAddress + OFS_DMA3SA), *(uint32_t *)&(storage[21]));
    __data16_write_addr((baseAddress + OFS_DMA3DA), *(uint32_t *)&(storage[23]));
    HWREG16(baseAddress + OFS_DMA3SZ)     = storage[25];
    HWREG16(baseAddress + OFS_DMA3CTL)    = storage[26];
    __data16_write_addr((baseAddress + OFS_DMA4SA), *(uint32_t *)&(storage[27]));
    __data16_write_addr((baseAddress + OFS_DMA4DA), *(uint32_t *)&(storage[29]));
    HWREG16(baseAddress + OFS_DMA4SZ)     = storage[31];
    HWREG16(baseAddress + OFS_DMA4CTL)    = storage[32];
    __data16_write_addr((baseAddress + OFS_DMA5SA), *(uint32_t *)&(storage[33]));
    __data16_write_addr((baseAddress + OFS_DMA5DA), *(uint32_t *)&(storage[35]));
    HWREG16(baseAddress + OFS_DMA5SZ)     = storage[37];
    HWREG16(baseAddress + OFS_DMA5CTL)    = storage[38];

    return;
}

#endif //__MSP430_HAS_DMAX_6__

...
void ctpl_CRC32_save(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    storage[2] = HWREG16(baseAddress + OFS_CRC32INIRESW1);
    storage[1] = HWREG16(baseAddress + OFS_CRC32INIRESW0);
    storage[0] = HWREG16(baseAddress + OFS_CRC16INIRESW0);
...
void ctpl_CRC32_restore(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    HWREG16(baseAddress + OFS_CRC16INIRESW0) = storage[0];
    HWREG16(baseAddress + OFS_CRC32INIRESW0) = storage[1];
    HWREG16(baseAddress + OFS_CRC32INIRESW1) = storage[2];

    return;
}

#endif //__MSP430_HAS_CRC32__

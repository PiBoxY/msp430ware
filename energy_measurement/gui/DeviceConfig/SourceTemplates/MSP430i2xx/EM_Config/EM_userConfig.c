...
HAL_ADC_SD24_Channel adcChannelArray[EM_CHANNEL_LENGTH] =
{
...
HAL_ADC_SD24_Configuration halAdcConfig =
{
    ...
    .referenceSelect = SD24_REF_INTERNAL,
...
// Global buffer to store the TLV values
uint8_t g_infoATLV[SYSTEM_NONV_INFOA_TLV_SIZE];
                                     ...
                                     uint8_t *pFlashCalibration)
{
    ...
    // Copy TLV values to RAM
    memcpy(g_infoATLV,
                   (uint8_t *) SYSTEM_NONV_INFOA_TLV_ADDR,
                   SYSTEM_NONV_INFOA_TLV_SIZE);
    ...
    // Store the TLV values back to flash
    FlashCtl_write8(
                    g_infoATLV,
                    (uint8_t *) SYSTEM_NONV_INFOA_TLV_ADDR,
                    SYSTEM_NONV_INFOA_TLV_SIZE

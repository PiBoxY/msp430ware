...
void HMI_sendPhaseResults(uint8_t phaseIdx, uint8_t phaseID)
{
<EMDC_HMI_SEND_PHASE_RESULT>
}

void HMI_sendTotalPhaseResults()
{
<EMDC_HMI_SEND_TOTAL_PHASE_RESULT>
}

void HMI_processPhaseResults(uint8_t resultStatus)
{
<EMDC_HMI_PROCESS_PHASE_RESULTS>	
}

void HMI_sendCalibValues()
{
<EMDC_HMI_SEND_CALIB_VALUES>
}

void HMI_updatePhaseIDx()
{
    switch(gCommandHandler.calibPhaseID)
    {
<EMDC_HMI_PHASE_IDX>
    }
}
...
void HMI_stateMachine(uint8_t resultStatus)
{
    ...
    gHMI = (HMI_STATE_t) gCommandHandler.appMode;

    if(COMMAND_HANDLER_CALIB_REQ_IN_PROGRESS == gCommandHandler.calibReqUpdate)
    {
        HMI_sendCalibValues();
        // Clear calibration req update flag
        gCommandHandler.calibReqUpdate = COMMAND_HANDLER_CALIB_REQ_IDLE;
    }

    switch(gHMI)
    {
        case HMI_STATE_IDLE:
            // No results sent to EMDC
            break;
        case HMI_STATE_ACTIVE:
            HMI_processPhaseResults(resultStatus);
            break;
        case HMI_STATE_CALIBRATE:

            // Check if phase idx needs to be updated
            if(gCommandHandler.calibPhaseIdxFlag)
            {
                // Clear phase idx flag
                gCommandHandler.calibPhaseIdxFlag = false;
                HMI_updatePhaseIDx();
            }

            // Check if the preload values need to be updated
            if(gCommandHandler.calibSetPreloadFlag)
            {
                // Clear phase preload flag
                gCommandHandler.calibSetPreloadFlag = false;

                // Update preload values & re initialize the conversions
                __disable_interrupt();
            ...
            // Check if the calibration values must be written memory
            if(gCommandHandler.calibReqSaveFlag)
            {
                // Clear calib req save flag
                gCommandHandler.calibReqSaveFlag = false;

                // Write calibration to flash memory
                EM_updateFlashCalibrationValues((uint8_t *)g_emPhaseCalibration,
                                                (uint8_t *)g_savedEmPhaseCalibration);

                // Send Flash Write complete to host
                CommandHandler_transmitCalibrationSave(&gstxPacket);
            }

            if(gCommandHandler.calibPhaseID > 0 && (gCommandHandler.calibPhaseID & resultStatus))
            {
